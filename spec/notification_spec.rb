...
require "spec_helper"
require_relative "../lib/teleflow"

RSpec.describe Teleflow::Api::Notification do
  let(:access_token) { "1234567890" }
  let(:client) { Teleflow::Client.new(access_token: access_token) }
  let(:base_uri) { "https://api-teleflow.khulnasoft.com/v1" }

  describe "#notifications" do
    let(:query_params) { { channels: ["email"], templates: ["63f731fb0c5d5e4967c52775"], page: 0 } }
    let(:response_body) { { notifications: [{ id: 1, message: "Welcome to Teleflow!" }] } }
